...
\redout{[Replace example below with air pollution example. Start using causal diagrams \emph{now}, e.g. chains, $X \to Y \to Z$, forks $X \leftarrow Z \to Y$, colliders $X \to W \leftarrow Y \to Z$.]} % Do we need to talk about fine-tuning? We should if the goal of the paper is to demonstrate ``this is how one builds and tests scientific theories of cities.''

\redout{[What is a correlation? One can also think of it as the amount of shared information between two random variables. This is what allows quantum correlations to be ``stronger'' than perfect classical correlations.]}

\redout{[Example involving Berkson's paradox?]}
